...
<div class="container px-6 mx-auto grid">
    ...
    <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
        ...
        <div class="flex items-center">
            ...
            <span>Edit Berita Alusista</span>
        </div>
    ...
    <form action="{{route('berita.update',$berita->id) }})}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" id="judul_berita" name="judul_berita"
                    ...
                    placeholder="test" value="{{$berita->judul_berita}}">
            </div>
            <div>
                <label for="penulis"
                ...
                <input type="text" id="penulis" name="penulis"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Doe" value="{{$berita->penulis}}">
            </div>
            <div>
                <label for="tanggal_publikasi"
                    class="block mb-2 text-sm font-medium text-gray-900">Tanggal Publikasi</label>
                <input type="date" id="tanggal_publikasi" name="tanggal_publikasi"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Doe" value="{{$berita->tanggal_publikasi}}">
            </div>
            ...
            <div>
                ...
                <select id="kategori" name="kategori"
                    ...
                    <option value="pertahanan_nasional" {{ $berita->kategori == 'pertahanan_nasional' ? 'selected' :
                        ''}} >Pertahanan Nasional</option>
                    <option value="teknologi_militer" {{ $berita->kategori == 'teknologi_militer' ? 'selected' : ''}}
                        >Teknologi Militer</option>
                    <option value="kerjasama_internasional:" {{ $berita->kategori == 'kerjasama_internasional' ?
                        'selected' : ''}} >Kerjasama Internasional:</option>
                    <option value="latihan_militer" {{ $berita->kategori == 'latihan_militer' ? 'selected' : ''}}
                        >Latihan Militer</option>
                </select>
        ...
        <div class="mb-5">
            ...
            @if($berita->gambar)
            <img src="{{asset('storage/'.$berita->gambar)}}" class="w-80 rounded-md img-preview" alt="image">
            @else
        ...
        <div class="mb-3">
            <div>
                ...
                <input id="deskripsi_berita" type="hidden" name="deskripsi_berita" required
                    value="{!!html_entity_decode($berita->deskripsi_berita)!!}">
