...
<article class="max-w-2xl px-6 py-24 mx-auto space-y-12 dark:bg-gray-800 dark:text-gray-50">
    <div class="w-full mx-auto space-y-4 text-center">
        <p class="text-xs font-semibold tracki uppercase"> {{ $berita->kategori }} </p>
        <h1 class="text-4xl font-bold leadi md:text-5xl">{{ $berita->judul_berita }}
        </h1>
        <p class="text-sm dark:text-gray-400">by
            <a rel="noopener noreferrer" href="#" target="_blank" class="underline dark:text-violet-400">
                <span itemprop="name">{{ $berita->penulis }}</span>
            </a>on
            <time datetime="2021-02-12 15:34:18-0200">{{
                \Carbon\Carbon::parse($berita->tanggal_publikasi)->format('d
                F Y') }}</time>
        </p>
    </div>
    <div class="dark:text-gray-100">
        <p class="dark:text-gray-400">{!!html_entity_decode($berita->deskripsi_berita)!!}</p>
    </div>
    <div class="pt-12 border-t dark:border-gray-700">
        <div class="flex flex-col space-y-4 md:space-y-0 md:space-x-6 md:flex-row">
            ...
            <div class="flex flex-col">
                <h4 class="text-lg font-semibold">{{ $berita->penulis }}</h4>
                <p class="dark:text-gray-400">Berita tentang alusista</p>
            </div>
        </div>
